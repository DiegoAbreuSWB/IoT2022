...
model = keras.Sequential([
    ...
    #keras.layers.Dense(2, activation='softmax')
     keras.layers.Dense(6, activation='softmax') #5=classes multiclass
])
model.compile("adam", "sparse_categorical_crossentropy", metrics=["accuracy"])

#data = pd.read_csv ('bases/boruncambin.csv')#binario
data = pd.read_csv ('../bases/4.eufyhome_multiclass.csv')#binario
